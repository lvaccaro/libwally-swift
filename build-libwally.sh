...
if [ $simulator == 1 ]; then
  if [ ! -d "build" ]; then
    ...
    set -v
    export CFLAGS="-O3 -arch x86_64 -arch i386 -mios-simulator-version-min=10.0 -isysroot `xcrun -sdk iphonesimulator --show-sdk-path`"
    export CXXFLAGS="-O3 -arch x86_64 -arch i386 -mios-simulator-version-min=10.0 -isysroot `xcrun -sdk iphonesimulator --show-sdk-path`"
    mkdir -p build
...
if [ $device == 1 ]; then
  ...
  if [ ! -d "build" ] || [ $clean == 1 ]; then
    ...
    set -v
    export CFLAGS="-O3 -arch arm64 -arch arm64e -arch armv7 -arch armv7s -mios-version-min=10.0 -isysroot `xcrun -sdk iphoneos --show-sdk-path`"
    export CXXFLAGS="-O3 -arch arm64 -arch arm64e -arch armv7 -arch armv7s -isysroot -mios-version-min=10.0 -isysroot `xcrun -sdk iphoneos --show-sdk-path`"
    mkdir -p build
